...
Ext.define('CustomApp', {
    ...
    config: {
        ...
        wsjfCalcFields:[   
            ...
            { field: 'PlanEstimate',      name: 'Plan Estimate',     menu: 'wsjfBulkSetPlan',  aboveLine: false },
            { field: 'JobSize',           name: 'Job Size',          menu: 'wsjfBulkSetSize',  aboveLine: false }
        ]
    },
                ...
                load: function(store) {
                    if (gApp.getSetting('useWSJFOverLoad')) {
                        ...
                        _.each(records, function(record) {
                            if ( record.hasField(wsjfScore)){
                                var num = gApp._calcWSJF(record).toFixed(2);
                                if ( num !== (record.get(wsjfScore) || 0).toFixed(2)) {
                                    record.set(wsjfScore,num);
                                    clearLoading = false;
                                }
                            }
                        });
                        if (clearLoading) { Ext.getCmp('headerBox').setLoading(false); }
...
Ext.define('wsjfBulkSetPlan', {
    ...
    config: {
        ...
        handler: function(menu, arg2, arg3) {
            ...
            if (menu.records[0].data._type.search("portfolio" > 0)){
